...
public:
    ...
    void insert(const pair<uint32_t,uint32_t>& pkt,uint32_t &oldVal, uint32_t &newVal,uint32_t HASH_SEED){
    ...
    static void getIdxVal(const pair<uint32_t,uint32_t>& pkt,uint32_t &regIdx,uint32_t &rhoVal,uint32_t HASH_SEED){
        uint32_t hashValue1=0;
        MurmurHash3_x86_32(&pkt,8,HASH_SEED, &hashValue1);
...
public:
    ...
    void insert(const pair<uint32_t,uint32_t>& pkt,uint32_t HASH_SEED) {
        ...
        if(hashTable.find(key)!=hashTable.end()){
            ...
            HLL.insert(pkt,oldVal,newVal,HASH_SEED);
            ...
            uint32_t oldVal,newVal;
            HLLs[HLLIdx].insert(pkt,oldVal,newVal,HASH_SEED);
            ...
            HyperLogLog<REG_NUM>::getIdxVal(pkt,regIdx,rhoVal,HASH_SEED);
...
//    //150KB
    const uint32_t K_150KB=500+188;
    const uint32_t PZ_SIZE_150KB=189;
//    //250KB
    const uint32_t K_250KB=500+481;
    const uint32_t PZ_SIZE_250KB=481;
...
// //     const char* traceFileDirMul=R"(../data/Mul/)";
//     const char* traceFileDirMul=R"(../data/Mul2020.4/)";
    ...
    processMultipleTracesAndGetAvgMetrics<K_100KB,PZ_SIZE_100KB,REG_NUM,WEIGHT_TH,PKT_MARGIN>(traceFileDir2019,fileNum,step,pktsOfFiles,actualFlowSpreadsOfFiles,actualItemNumOfFiles,topKs,outputDirPath,"2019");
    processMultipleTracesAndGetAvgMetrics<K_200KB,PZ_SIZE_200KB,REG_NUM,WEIGHT_TH,PKT_MARGIN>(traceFileDir2019,fileNum,step,pktsOfFiles,actualFlowSpreadsOfFiles,actualItemNumOfFiles,topKs,outputDirPath,"2019");
    processMultipleTracesAndGetAvgMetrics<K_300KB,PZ_SIZE_300KB,REG_NUM,WEIGHT_TH,PKT_MARGIN>(traceFileDir2019,fileNum,step,pktsOfFiles,actualFlowSpreadsOfFiles,actualItemNumOfFiles,topKs,outputDirPath,"2019");
    processMultipleTracesAndGetAvgMetrics<K_150KB,PZ_SIZE_150KB,REG_NUM,WEIGHT_TH,PKT_MARGIN>(traceFileDir2019,fileNum,step,pktsOfFiles,actualFlowSpreadsOfFiles,actualItemNumOfFiles,topKs,outputDirPath,"2019");
    processMultipleTracesAndGetAvgMetrics<K_250KB,PZ_SIZE_250KB,REG_NUM,WEIGHT_TH,PKT_MARGIN>(traceFileDir2019,fileNum,step,pktsOfFiles,actualFlowSpreadsOfFiles,actualItemNumOfFiles,topKs,outputDirPath,"2019");
